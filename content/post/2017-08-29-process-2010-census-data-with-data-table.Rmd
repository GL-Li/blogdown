...
This geographic data includes all summary levels. As we only want the census block level data, we further filt with `SUMLEV == "100"`. At this stage we get the geographic data we want.
...
Are we sure we get the correct geographic data? We can plot the latitude and longitude directly on a map. From the map below you will also get a sense of what a census block is; it is basicly a street block in urban area. We need internet connection to download map data from Google Map.
...
It has 199 coloumns; which ones are what we want? We still go back to the techincal documentation, page 6-22, and read the description for file 03. The 5th field is logical record number, 6th the total population data and 8th the black population data. Each population data field also has a reference but is hard to follow. For clarity we name these coloumn in plain English. So all the data we need in Indiana is
```{r}
...
## More concise code
### the data.table way
The above codes that select South Bend data can be squeezed with pipe operator `%>%` to get rid of intermeidate variables.
